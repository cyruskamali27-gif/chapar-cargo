...
export default function ChaparFormSimple({ product, onSubmit }) {
  ...
  const field = "w-full rounded-xl border border-white/12 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none placeholder:text-white/50 focus:border-cyan-400/60 transition";
  const lab = "mb-1.5 block text-xs text-white/50";
  ...
  return (
    <div dir="rtl" className="p-5 font-sans">
      <div className="flex items-center gap-2.5 border-b border-white/10 pb-4">
        ...
        <div className="flex-1"><div className="text-sm font-bold text-white">{product?.title || `${product?.brand || ""} ${product?.model || ""}` || "محصولِ تأییدشده"}</div><div className="text-[11px] text-white/50">{[product?.brand, product?.country, "تأییدشده"].filter(Boolean).join(" · ")}</div></div>
      ...
      <div className="space-y-3.5">
        ...
        <div><label className={lab}>ایمیلِ گیرنده <span className="text-white/50">(اختیاری)</span></label><input value={f.email} onChange={(e) => set("email", e.target.value)} placeholder="recipient@example.com" className={field} /></div>
